...
export class SphereGeometry extends Geometry {
    ...
    constructor(values: Record<string, unknown> = {}) {
        ...
        this.data = {
            radius, latSegments: lat, lonSegments: lon,
            vertices, indices,
    ...
    getDescriptors(): readonly GPUDescriptor[] {
        return [
            { id: 'vertices', role: 'vertex', schema: SphereGeometry.vertexStruct, count: this.data['vertexCount'] as number },
...
function generateSphere(radius: number, lat: number, lon: number): { vertices: Float32Array; indices: Uint16Array } {
    const verts: number[] = [];
    const idx: number[] = [];
    for (let i = 0; i <= lat; i++) {
