/**
 * Kernel WGSL: pbf_position_correct — aplica correção de posição Δxᵢ.
 *
 * Para cada partícula i:
 *   s_corr = −k · (W(r,h) / W(Δq,h))^n    [repulsão artificial anti-clustering]
 *   Δxᵢ = (1/ρ₀) · Σⱼ (λᵢ + λⱼ + s_corr) · ∇W(xᵢ−xⱼ, h)
 *   pos[i] += Δxᵢ
 *
 * Bind groups:
 ...
 */
export const WGSL_KERNEL_PBF_POSITION_CORRECT = /* wgsl */`
