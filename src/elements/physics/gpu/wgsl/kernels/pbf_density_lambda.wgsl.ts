/**
 * Kernel WGSL: pbf_density_lambda — computa densidade ρᵢ e multiplicador λᵢ.
 *
 * Para cada partícula i:
 *   ρᵢ = Σⱼ∈N(i) W(|xᵢ−xⱼ|, h)                     [densidade SPH]
 *   Cᵢ = ρᵢ/ρ₀ − 1                                    [constraint de incompressibilidade]
 *   λᵢ = −Cᵢ / (Σⱼ ‖∇Cᵢⱼ‖² + ε)                    [multiplicador XPBD]
 *   Escreve λᵢ em particles[i].pos.w
 *
 * Kernel W: Poly6 W(r,h) = (315/64πh⁹) * max(h²−r², 0)³
 * Kernel ∇W: Spiky ∂W/∂r = −(45/πh⁶) * max(h−r, 0)²
 *
 * Bind groups:
 ...
 */
export const WGSL_KERNEL_PBF_DENSITY_LAMBDA = /* wgsl */`
