/**
 * Kernel WGSL: ns_find — busca de vizinhos por raio h para cada partícula.
 *
 * Para cada partícula i:
 *   1. Determina célula base (ix, iy, iz)
 *   2. Itera stencil 3×3×3 ao redor da célula base
 *   3. Para cada partícula j no sorted da célula vizinha:
 *      - Calcula distância r = |pos_i - pos_j|
 *      - Se r < h e i ≠ j: adiciona j ao neighbor_list[i]
 *   4. Escreve neighbor_count[i]
 *
 * Trunca em max_neighbors (counts.z) se houver mais vizinhos que o buffer comporta.
 *
 * Bind groups:
 *   @group(0) @binding(0) — NsSimParams (uniform)
 *   @group(1) @binding(0) — particle_data: array<f32> (read)
 *   @group(1) @binding(1) — cell_start: array<u32> (read)
 *   @group(1) @binding(2) — cell_count: array<u32> (read)
 *   @group(1) @binding(3) — sorted_particles: array<u32> (read)
 *   @group(1) @binding(4) — neighbor_list: array<u32> (read_write)
 *   @group(1) @binding(5) — neighbor_count: array<u32> (read_write)
 *
 * Dispatch: ceil(particle_count / 64) workgroups.
 */
export const WGSL_KERNEL_NS_FIND = /* wgsl */`
