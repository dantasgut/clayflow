/**
 * Kernel WGSL: pbf_vorticity — computa curl(vᵢ) e aplica força de vorticity confinement.
 *
 * Para cada partícula i:
 *   ωᵢ = Σⱼ vᵢⱼ × ∇W(xᵢ−xⱼ, h)      [curl da velocidade]
 *   ηᵢ = Σⱼ ‖ωⱼ‖ · ∇W(xᵢ−xⱼ, h)    [gradiente da magnitude de vorticity]
 *   Nᵢ = ηᵢ / ‖ηᵢ‖                    [normalizado]
 *   f_vort = ε_v · (Nᵢ × ωᵢ)
 *   vel[i] += dt · f_vort
 *   Escreve ωᵢ em particles[i].curl.xyz
 *
 * Bind groups:
 ...
 */
export const WGSL_KERNEL_PBF_VORTICITY = /* wgsl */`
