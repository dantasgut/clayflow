...
export class PBFFlow extends Flow {
    ...
    private ensureGpuObjects(): void {
        ...
        if (this.densityLambdaShader === null)
            this.densityLambdaShader = this.core.create<ShaderModuleSpec>({
                kind: 'shader',
                discriminator: 'pbf_density_lambda',
                source: b + '\n' + pbfDensityLambdaKernel,
            });
        if (this.positionCorrectShader === null)
            this.positionCorrectShader = this.core.create<ShaderModuleSpec>({
                kind: 'shader',
                discriminator: 'pbf_position_correct',
                source: b + '\n' + pbfPositionCorrectKernel,
            });
        if (this.velocityUpdateShader === null)
            this.velocityUpdateShader = this.core.create<ShaderModuleSpec>({
                kind: 'shader',
                discriminator: 'pbf_velocity_update',
                source: b + '\n' + pbfVelocityUpdateKernel,
            });
        if (this.xsphShader === null)
            this.xsphShader = this.core.create<ShaderModuleSpec>({
                kind: 'shader',
                discriminator: 'pbf_xsph',
                source: b + '\n' + pbfXsphKernel,
            });
        if (this.vorticityShader === null)
            this.vorticityShader = this.core.create<ShaderModuleSpec>({
                kind: 'shader',
        ...
        if (this.paramsLayout === null)
            this.paramsLayout = this.core.create<LayoutSpec>({
                kind: 'layout',
                discriminator: 'pbf_params_layout',
                entries: [
                    {
                        binding: 0,
        ...
        if (this.particlesLayout === null)
            this.particlesLayout = this.core.create<LayoutSpec>({
                kind: 'layout',
                discriminator: 'pbf_particles_layout',
                entries: [
                    {
                        binding: 0,
                        visibility: GPUShaderStage.COMPUTE,
        ...
        if (this.neighborsLayout === null)
            this.neighborsLayout = this.core.create<LayoutSpec>({
                kind: 'layout',
                discriminator: 'pbf_neighbors_layout',
                entries: [
                    {
                        binding: 0,
                        visibility: GPUShaderStage.COMPUTE,
        ...
        if (this.collidersLayout === null)
            this.collidersLayout = this.core.create<LayoutSpec>({
                kind: 'layout',
                discriminator: 'pbf_colliders_layout',
                entries: [
                    {
                        binding: 0,
                        visibility: GPUShaderStage.COMPUTE,
        ...
        if (this.particlesBg === null)
            this.particlesBg = this.core.create<BindGroupSpec>({
                kind: 'bindgroup',
                discriminator: 'pbf_particles_bg',
        ...
        if (
            ...
            this.neighborsBg = this.core.create<BindGroupSpec>({
                kind: 'bindgroup',
                discriminator: 'pbf_neighbors_bg',
        ...
        if (this.collidersBg === null)
            this.collidersBg = this.core.create<BindGroupSpec>({
                kind: 'bindgroup',
                discriminator: 'pbf_colliders_bg',
        ...
        if (this.predictPipeline === null)
            this.predictPipeline = this.core.create<ComputePipelineSpec>({
                kind: 'pipeline',
                subkind: 'compute',
                discriminator: 'pbf_predict_pipeline',
                layouts: layoutsNoNeigh,
                shader: this.predictShader,
                entryPoint: 'pbf_predict_main',
            });
        if (this.densityLambdaPipeline === null)
            this.densityLambdaPipeline = this.core.create<ComputePipelineSpec>({
                kind: 'pipeline',
                subkind: 'compute',
                discriminator: 'pbf_density_lambda_pipeline',
                layouts: layoutsWithNeigh,
                shader: this.densityLambdaShader,
                entryPoint: 'pbf_density_lambda_main',
            });
        if (this.positionCorrectPipeline === null)
            this.positionCorrectPipeline = this.core.create<ComputePipelineSpec>({
                kind: 'pipeline',
                subkind: 'compute',
                discriminator: 'pbf_position_correct_pipeline',
                layouts: layoutsWithNeigh,
                shader: this.positionCorrectShader,
                entryPoint: 'pbf_position_correct_main',
            });
        if (this.velocityUpdatePipeline === null)
            this.velocityUpdatePipeline = this.core.create<ComputePipelineSpec>({
                kind: 'pipeline',
                subkind: 'compute',
                discriminator: 'pbf_velocity_update_pipeline',
                layouts: layoutsNoNeigh,
                shader: this.velocityUpdateShader,
                entryPoint: 'pbf_velocity_update_main',
            });
        if (this.xsphPipeline === null)
            this.xsphPipeline = this.core.create<ComputePipelineSpec>({
                kind: 'pipeline',
                subkind: 'compute',
                discriminator: 'pbf_xsph_pipeline',
                layouts: layoutsWithNeigh,
                shader: this.xsphShader,
                entryPoint: 'pbf_xsph_main',
            });
        if (this.vorticityPipeline === null)
            this.vorticityPipeline = this.core.create<ComputePipelineSpec>({
                kind: 'pipeline',
                subkind: 'compute',
                discriminator: 'pbf_vorticity_pipeline',
                layouts: layoutsWithNeigh,
                shader: this.vorticityShader,
        ...
        if (this.collisionPipeline === null)
            this.collisionPipeline = this.core.create<ComputePipelineSpec>({
                kind: 'pipeline',
                subkind: 'compute',
                discriminator: 'pbf_collision_pipeline',
    ...
    dispatch(frame: Frame): void {
        ...
        if (
            ...
            || this.densityLambdaPipeline === null
            || this.positionCorrectPipeline === null
            || this.velocityUpdatePipeline === null
            || this.xsphPipeline === null
            || this.vorticityPipeline === null
        ...
        for (let s = 0; s < this.substeps; s++) {
            ...
            frame.compute('PBFFlow.predict', (pass) => {
                pass.bind
                    .setPipeline(this.predictPipeline!)
            ...
            for (let i = 0; i < this.solverIters; i++) {
                frame.compute('PBFFlow.density_lambda', (pass) => {
                    pass.bind
                        .setPipeline(this.densityLambdaPipeline!)
                        .setBindGroup(0, this.paramsBg!)
                        .setBindGroup(1, this.particlesBg!)
                ...
                frame.compute('PBFFlow.position_correct', (pass) => {
                    pass.bind
                        .setPipeline(this.positionCorrectPipeline!)
                        .setBindGroup(0, this.paramsBg!)
                        .setBindGroup(1, this.particlesBg!)
                        .setBindGroup(2, this.neighborsBg!);
                    pass.dispatch.workgroups(wgs);
            ...
            frame.compute('PBFFlow.velocity_update', (pass) => {
                pass.bind
                    .setPipeline(this.velocityUpdatePipeline!)
                    .setBindGroup(0, this.paramsBg!)
                    .setBindGroup(1, this.particlesBg!);
                pass.dispatch.workgroups(wgs);
            });
            frame.compute('PBFFlow.vorticity', (pass) => {
                pass.bind
                    .setPipeline(this.vorticityPipeline!)
                    .setBindGroup(0, this.paramsBg!)
                    .setBindGroup(1, this.particlesBg!)
                    .setBindGroup(2, this.neighborsBg!);
                pass.dispatch.workgroups(wgs);
