/**
 * Kernel WGSL: rb_sync_transform — sincroniza posição/rotação GPU → UBO do renderer.
 *
 * Para cada corpo rígido GPU-simulado, lê `pos_pred` e `rot_pred` do buffer global,
 * constrói uma mat4x4f TRS e escreve diretamente no slot correspondente do UBO
 * dinâmico de modelo do renderer (`renderer_object_dyn_ubo`).
 *
 * ## Bindings (@group 0)
 *
 * | binding | tipo                    | conteúdo                           |
 * |---------|-------------------------|------------------------------------|
 * |    0    | uniform                 | RBSimParams                        |
 * |    1    | storage read            | RigidBody[] (gpu_rb_bodies)        |
 * |    2    | storage read            | u32[] rb_to_ubo_map (gpuIdx→slot)  |
 * |    3    | storage read_write      | vec4f[] renderer_object_dyn_ubo    |
 *
 * ## UBO stride
 *
 * O renderer usa stride de 256 bytes por slot (64 floats = 16 vec4f).
 * A mat4x4f ocupa os primeiros 64 bytes (4 colunas × 16 bytes).
 * Base de escrita: `ubo_slot * 16` (índice vec4f).
 *
 * Depende de: WGSL_STRUCT_RB_SIM_PARAMS, WGSL_STRUCT_RIGID_BODY, WGSL_MAT
 */
export const WGSL_KERNEL_RB_SYNC_TRANSFORM = /* wgsl */`
