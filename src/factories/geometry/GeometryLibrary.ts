...
export class GeometryLibrary {
    private static _cache = new Map<string, Geometry>();

    public static get<T extends Geometry>(key: string, factory: () => T): T {
        if (!GeometryLibrary._cache.has(key)) {
            GeometryLibrary._cache.set(key, factory());
        }
        return GeometryLibrary._cache.get(key) as T;
    }

    public static has(key: string): boolean {
        return GeometryLibrary._cache.has(key);
    }

    /** Remove uma entrada do cache (ex: ao descarregar um nível). */
    public static evict(key: string): void {
        GeometryLibrary._cache.delete(key);
    }

    public static clear(): void {
        GeometryLibrary._cache.clear();
    }
}
